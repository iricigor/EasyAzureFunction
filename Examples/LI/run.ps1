...
try {
  ...
  $EzNumChars = $requestBody.EzNumChars
} catch {$Failed = $true}
if ($Failed) {
...
if (!$EzInvokeCommand) {
  ...
  try {
    $ParamsHash = @{}
    if ($EzNumChars) {$ParamsHash.Add('NumChars',$EzNumChars)}
    "Params: $($ParamsHash.Keys -join `",`")"
    $Output = .\LoremIpsum.ps1 @ParamsHash | Out-String
  ...
  $Head = "<head><style>body {background-color: #012456; color: $Color;}</style><title>EasyAzureFunction - .\LoremIpsum.ps1 running example</title></head>"
